...
class DenseUNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.TransDowns = nn.ModuleDict([
            (f"TD{i}", DenseUNet._trans_down(
                2*features, features, drop_rate=drop_rate))
            for i in range(depth)
        ])

        self.bottleneck = DenseUNet._bottleneck(
            features, layers=default_layers, growth_rate=growth_rate)

        self.TransUps = nn.ModuleDict(
            [
                (f"TU{depth-1}", DenseUNet._trans_up(2*features, features))
            ]+[
                (f"TU{i}", DenseUNet._trans_up(4*features, features))
    ...
    def forward(self, x):
        ...
        # encode
        links = []
        for denseblock, trans_down in zip(self.DenseBlockEncoders.values(), self.TransDowns.values()):
            x = denseblock(x)
            links.append(x)
            x = trans_down(x)
        ...
        x = self.bottleneck(x)
        # decode
        for denseblock, trans_up in zip(self.DenseBlockDecoders.values(), self.TransUps.values()):
            x = trans_up(x)
        ...
        def __init__(self, in_channels, layers=4,
            ...
            self.composite_layers = nn.ModuleList([
                DenseUNet._dense_block._composite(
                    in_channels+i*growth_rate, growth_rate, drop_rate) for i in range(layers)
            ])
